...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-blue-900/30 bg-navy-900/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="font-extrabold text-lg text-white tracking-tight">
                  Match<span className="text-cyan-400">Jurídico</span>
                </span>
              ...
              <p className="text-[10px] text-slate-400 font-medium">Assistência Judiciária & IA</p>
            </div>
          </Link>
          ...
          {/* Seletor de Perfil Ativo & GOV.br Badge */}
          <div className="hidden lg:flex items-center space-x-3">
            
            {/* Simulador de Perfil Demo */}
            <div className="flex items-center bg-navy-850 p-1 rounded-xl border border-blue-900/40">
              <button
            ...
            {/* Gov.br Badge */}
            <div className="flex items-center space-x-2 px-2.5 py-1 rounded-lg bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 text-xs font-medium">
            ...
            <Link
              href="/auth/login"
              className="px-3 py-1.5 text-xs font-medium text-slate-300 hover:text-white bg-slate-800/80 hover:bg-slate-700 rounded-lg border border-slate-700 transition-all"
            >
              Entrar / Trocar
            </Link>
          </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-blue-900/40 bg-navy-900/95 px-4 pt-2 pb-6 space-y-3">
          <div className="flex justify-between items-center pb-2 border-b border-slate-800">
            <span className="text-xs text-slate-400">Modo de Acesso:</span>
            <div className="flex space-x-2">
              <button
                onClick={() => handleSwitchRole('cidadao')}
                className={`px-3 py-1 rounded-md text-xs font-medium ${profile.role === 'cidadao' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}
              >
                Cidadão
              </button>
              <button
                onClick={() => handleSwitchRole('advogado')}
                className={`px-3 py-1 rounded-md text-xs font-medium ${profile.role === 'advogado' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400'}`}
              >
                Advogado
              </button>
            </div>
          ...
          <Link 
            ...
            className="flex items-center space-x-2 py-2 text-sm text-cyan-300 font-medium"
          >
          ...
          <Link 
            href="/cidadao/meus-casos" 
            onClick={() => setMobileMenuOpen(false)}
            className="flex items-center space-x-2 py-2 text-sm text-slate-200"
          >
            <User className="w-4 h-4 text-blue-400" />
